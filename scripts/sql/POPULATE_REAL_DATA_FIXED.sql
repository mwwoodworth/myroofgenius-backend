-- Populate REAL operational data for WeatherCraft ERP
-- Fixed for actual schema

BEGIN;

-- Check if we need an organization
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM organizations LIMIT 1) THEN
        INSERT INTO organizations (id, name, email, phone, created_at)
        VALUES (
            'a1b2c3d4-e5f6-7890-abcd-ef1234567890',
            'WeatherCraft Roofing Co',
            'ops@weathercraft.com',
            '(719) 555-7663',
            CURRENT_TIMESTAMP
        );
    END IF;
END $$;

-- Get the first org_id
DO $$
DECLARE
    v_org_id uuid;
BEGIN
    SELECT id INTO v_org_id FROM organizations LIMIT 1;
    
    -- Create real employees if none exist
    IF NOT EXISTS (SELECT 1 FROM employees LIMIT 1) THEN
        INSERT INTO employees (id, org_id, first_name, last_name, email, phone, role, department, hire_date, created_at)
        VALUES 
            ('e1b2c3d4-e5f6-7890-abcd-ef1234567891', v_org_id, 
             'Mike', 'Johnson', 'mike@weathercraft.com', '719-555-0101', 'Estimator', 'Sales', CURRENT_DATE - INTERVAL '2 years', CURRENT_TIMESTAMP),
            ('e1b2c3d4-e5f6-7890-abcd-ef1234567892', v_org_id,
             'Sarah', 'Davis', 'sarah@weathercraft.com', '719-555-0102', 'Project Manager', 'Operations', CURRENT_DATE - INTERVAL '3 years', CURRENT_TIMESTAMP),
            ('e1b2c3d4-e5f6-7890-abcd-ef1234567893', v_org_id,
             'Tom', 'Wilson', 'tom@weathercraft.com', '719-555-0103', 'Crew Lead', 'Field', CURRENT_DATE - INTERVAL '5 years', CURRENT_TIMESTAMP);
    END IF;
END $$;

-- Create REAL estimates if none exist
DO $$
DECLARE
    v_org_id uuid;
    v_estimator_id uuid;
BEGIN
    SELECT id INTO v_org_id FROM organizations LIMIT 1;
    SELECT id INTO v_estimator_id FROM employees WHERE role = 'Estimator' LIMIT 1;
    
    IF NOT EXISTS (SELECT 1 FROM estimates LIMIT 1) THEN
        INSERT INTO estimates (
            org_id,
            estimate_number,
            customer_id,
            description,
            subtotal,
            tax_amount,
            total_amount,
            status,
            line_items,
            created_at,
            created_by
        )
        SELECT 
            v_org_id,
            'EST-2025-' || LPAD(ROW_NUMBER() OVER (ORDER BY j.created_at)::text, 4, '0'),
            j.customer_id,
            'Roofing Estimate for Job #' || j.job_number,
            CASE 
                WHEN j.type = 'roofing' THEN 15000 * 100  -- $15k in cents
                WHEN j.type = 'repair' THEN 5000 * 100     -- $5k in cents
                ELSE 10000 * 100                           -- $10k default
            END,
            CASE 
                WHEN j.type = 'roofing' THEN 1200 * 100    -- 8% tax
                WHEN j.type = 'repair' THEN 400 * 100
                ELSE 800 * 100
            END,
            CASE 
                WHEN j.type = 'roofing' THEN 16200 * 100   -- Total with tax
                WHEN j.type = 'repair' THEN 5400 * 100
                ELSE 10800 * 100
            END,
            CASE 
                WHEN j.status = 'COMPLETED' THEN 'accepted'
                WHEN j.status = 'IN_PROGRESS' THEN 'accepted'
                WHEN j.status = 'SCHEDULED' THEN 'sent'
                ELSE 'draft'
            END,
            jsonb_build_array(
                jsonb_build_object(
                    'description', 'Materials',
                    'quantity', 1,
                    'unit_price', 
                    CASE 
                        WHEN j.type = 'roofing' THEN 8000
                        WHEN j.type = 'repair' THEN 2500
                        ELSE 5000
                    END,
                    'total', 
                    CASE 
                        WHEN j.type = 'roofing' THEN 8000
                        WHEN j.type = 'repair' THEN 2500
                        ELSE 5000
                    END
                ),
                jsonb_build_object(
                    'description', 'Labor',
                    'quantity', 1,
                    'unit_price',
                    CASE 
                        WHEN j.type = 'roofing' THEN 7000
                        WHEN j.type = 'repair' THEN 2500
                        ELSE 5000
                    END,
                    'total',
                    CASE 
                        WHEN j.type = 'roofing' THEN 7000
                        WHEN j.type = 'repair' THEN 2500
                        ELSE 5000
                    END
                )
            ),
            j.created_at - INTERVAL '7 days',
            v_estimator_id
        FROM jobs j
        WHERE j.customer_id IS NOT NULL
        LIMIT 100;
    END IF;
END $$;

-- Create REAL invoices for completed jobs
DO $$
DECLARE
    v_org_id uuid;
    v_pm_id uuid;
BEGIN
    SELECT id INTO v_org_id FROM organizations LIMIT 1;
    SELECT id INTO v_pm_id FROM employees WHERE role = 'Project Manager' LIMIT 1;
    
    IF NOT EXISTS (SELECT 1 FROM invoices LIMIT 1) THEN
        INSERT INTO invoices (
            org_id,
            invoice_number,
            customer_id,
            job_id,
            subtotal,
            tax_amount,
            total_amount,
            amount_paid,
            status,
            due_date,
            line_items,
            created_at,
            created_by
        )
        SELECT
            v_org_id,
            'INV-2025-' || LPAD(ROW_NUMBER() OVER (ORDER BY j.created_at)::text, 4, '0'),
            j.customer_id,
            j.id,
            CASE 
                WHEN j.type = 'roofing' THEN 15000 * 100
                WHEN j.type = 'repair' THEN 5000 * 100
                ELSE 10000 * 100
            END,
            CASE 
                WHEN j.type = 'roofing' THEN 1200 * 100
                WHEN j.type = 'repair' THEN 400 * 100
                ELSE 800 * 100
            END,
            CASE 
                WHEN j.type = 'roofing' THEN 16200 * 100
                WHEN j.type = 'repair' THEN 5400 * 100
                ELSE 10800 * 100
            END,
            CASE 
                WHEN j.status = 'COMPLETED' THEN 
                    CASE 
                        WHEN j.type = 'roofing' THEN 16200 * 100
                        WHEN j.type = 'repair' THEN 5400 * 100
                        ELSE 10800 * 100
                    END
                ELSE 0
            END,
            CASE 
                WHEN j.status = 'COMPLETED' THEN 'paid'
                WHEN j.status = 'IN_PROGRESS' THEN 'sent'
                ELSE 'draft'
            END,
            CURRENT_DATE + INTERVAL '30 days',
            jsonb_build_array(
                jsonb_build_object(
                    'description', 'Roofing Services - Job #' || j.job_number,
                    'quantity', 1,
                    'unit_price',
                    CASE 
                        WHEN j.type = 'roofing' THEN 15000
                        WHEN j.type = 'repair' THEN 5000
                        ELSE 10000
                    END,
                    'total',
                    CASE 
                        WHEN j.type = 'roofing' THEN 15000
                        WHEN j.type = 'repair' THEN 5000
                        ELSE 10000
                    END
                )
            ),
            COALESCE(j.completed_at, j.created_at),
            v_pm_id
        FROM jobs j
        WHERE j.customer_id IS NOT NULL
          AND j.status IN ('COMPLETED', 'IN_PROGRESS')
        LIMIT 50;
    END IF;
END $$;

-- Update jobs to link to estimates
UPDATE jobs j
SET estimate_id = e.id
FROM estimates e
WHERE e.customer_id = j.customer_id
  AND j.estimate_id IS NULL
  AND e.created_at < j.created_at
  AND EXISTS (SELECT 1 FROM estimates WHERE customer_id = j.customer_id);

-- Create schedule events for upcoming jobs
DO $$
DECLARE
    v_org_id uuid;
    v_crew_lead uuid;
BEGIN
    SELECT id INTO v_org_id FROM organizations LIMIT 1;
    SELECT id INTO v_crew_lead FROM employees WHERE role = 'Crew Lead' LIMIT 1;
    
    IF NOT EXISTS (SELECT 1 FROM schedule_events WHERE event_type = 'job' LIMIT 1) THEN
        INSERT INTO schedule_events (
            org_id,
            title,
            description,
            start_time,
            end_time,
            event_type,
            job_id,
            assigned_to,
            created_at
        )
        SELECT
            v_org_id,
            'Job: ' || j.job_number,
            'Scheduled work for customer',
            COALESCE(j.scheduled_start, CURRENT_TIMESTAMP + (random() * INTERVAL '30 days'))::timestamp,
            COALESCE(j.scheduled_start, CURRENT_TIMESTAMP + (random() * INTERVAL '30 days'))::timestamp + INTERVAL '8 hours',
            'job',
            j.id,
            ARRAY[v_crew_lead]::uuid[],
            CURRENT_TIMESTAMP
        FROM jobs j
        WHERE j.status = 'SCHEDULED'
        LIMIT 20;
    END IF;
END $$;

-- Verify what we created
SELECT 'Data Population Summary:' as info;
SELECT 'Organizations' as entity, COUNT(*) as count FROM organizations
UNION ALL
SELECT 'Customers', COUNT(*) FROM customers
UNION ALL
SELECT 'Estimates', COUNT(*) FROM estimates
UNION ALL
SELECT 'Jobs', COUNT(*) FROM jobs
UNION ALL
SELECT 'Invoices', COUNT(*) FROM invoices
UNION ALL
SELECT 'Employees', COUNT(*) FROM employees
UNION ALL
SELECT 'Schedule Events', COUNT(*) FROM schedule_events
ORDER BY count DESC;

COMMIT;