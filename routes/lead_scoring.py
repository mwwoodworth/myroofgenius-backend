...
async def create_lead_scoring(
    ...
    query = """
        INSERT INTO lead_scoring (name, description, status, data)
        VALUES ($1, $2, $3, $4)
...
async def list_lead_scoring(
    ...
    query = "SELECT * FROM lead_scoring WHERE 1=1"
    params = []
    param_count = 0
...
async def get_lead_scoring(
    ...
    query = "SELECT * FROM lead_scoring WHERE id = $1"
...
async def delete_lead_scoring(
    ...
    query = "DELETE FROM lead_scoring WHERE id = $1 RETURNING id"
...
async def get_lead_scoring_stats(conn: asyncpg.Connection = Depends(get_db)):
