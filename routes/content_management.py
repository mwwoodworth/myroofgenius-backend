...
async def create_content_management(
    ...
    query = """
        INSERT INTO content_management (name, description, status, data)
        VALUES ($1, $2, $3, $4)
...
async def list_content_management(
    ...
    query = "SELECT * FROM content_management WHERE 1=1"
    params = []
    param_count = 0
...
async def delete_content_management(
    ...
    query = "DELETE FROM content_management WHERE id = $1 RETURNING id"
...
async def get_content_management_stats(conn: asyncpg.Connection = Depends(get_db)):
