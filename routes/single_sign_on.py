...
async def create_single_sign_on(
    ...
    query = """
        INSERT INTO single_sign_on (name, description, status, data)
        VALUES ($1, $2, $3, $4)
...
async def list_single_sign_on(
    ...
    query = "SELECT * FROM single_sign_on WHERE 1=1"
    params = []
    param_count = 0
...
async def get_single_sign_on(
    ...
    query = "SELECT * FROM single_sign_on WHERE id = $1"
...
async def delete_single_sign_on(
    ...
    query = "DELETE FROM single_sign_on WHERE id = $1 RETURNING id"
...
async def get_single_sign_on_stats(conn: asyncpg.Connection = Depends(get_db)):
