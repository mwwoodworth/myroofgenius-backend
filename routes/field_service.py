...
async def create_field_service(
    ...
    query = """
        INSERT INTO field_service (tenant_id, name, description, status, data)
        VALUES ($1, $2, $3, $4, $5)
...
async def list_field_service(
    ...
    query = "SELECT * FROM field_service WHERE tenant_id = $1"
    params = [tenant_id]
    param_count = 1
...
async def get_field_service(
    ...
    query = "SELECT * FROM field_service WHERE tenant_id = $1 AND id = $2"
...
async def delete_field_service(
    ...
    query = "DELETE FROM field_service WHERE tenant_id = $1 AND id = $2 RETURNING id"
...
async def get_field_service_stats(
    conn: asyncpg.Connection = Depends(get_db),
    current_user: dict = Depends(get_current_user)
):
