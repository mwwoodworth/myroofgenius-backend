...
async def create_audit_management(
    ...
    query = """
        INSERT INTO audit_management (name, description, status, data)
        VALUES ($1, $2, $3, $4)
...
async def list_audit_management(
    ...
    """List audit management records"""
    query = "SELECT * FROM audit_management WHERE 1=1"
    params = []
    param_count = 0
...
async def get_audit_management(
    ...
    """Get specific audit management record"""
    query = "SELECT * FROM audit_management WHERE id = $1"
...
async def update_audit_management(
    ...
    """Update audit management record"""
    if 'data' in updates:
...
async def delete_audit_management(
    ...
    """Delete audit management record"""
    query = "DELETE FROM audit_management WHERE id = $1 RETURNING id"
...
async def get_audit_management_stats(conn: asyncpg.Connection = Depends(get_db)):
    """Get audit management statistics"""
    query = """
        SELECT
            COUNT(*) as total,
