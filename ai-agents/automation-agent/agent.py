...
async def heartbeat():
    while True:
        ...
        logger.info(f"{'automation-agent'} heartbeat - Status: {agent_state.status}, Tasks: {agent_state.tasks_processed}")

@app.on_event("startup")
async def startup_event():
    asyncio.create_task(heartbeat())
    logger.info(f"{'automation-agent'} started on port 6003")
