...
class PersistentMemoryBrain:
    ...
    SECURITY: Legacy table creation (brain_memory, thought_stream, decision_log,
    pattern_library, knowledge_synthesis, system_state, learning_objectives) is
    DISABLED by default. These tables lack tenant_id and must never be recreated.
    All memory operations should use unified_ai_memory instead.
    Set ENABLE_LEGACY_BRAIN=1 to explicitly opt in (development only).
    """

    _LEGACY_ENABLED = os.getenv("ENABLE_LEGACY_BRAIN", "").strip() == "1"
    ...
    def __init__(self):
        ...
        if not self._LEGACY_ENABLED:
            logger.info(
                "PersistentMemoryBrain: legacy brain DISABLED (ENABLE_LEGACY_BRAIN != 1). "
                "All memory operations route to unified_ai_memory."
            )
            return

        # Legacy path — only reachable with explicit opt-in
    ...
    async def _initialize_brain(self):
        ...
        SECURITY: Only reachable when ENABLE_LEGACY_BRAIN=1 (guarded in __init__).
        Defense-in-depth: bail out here too if the flag is not set."""
        if not self._LEGACY_ENABLED:
            logger.warning(
                "_initialize_brain called but legacy brain is disabled. No-op."
            )
            return
        try:
    ...
    async def store_thought(self, thought: Dict, importance: float = 0.5):
        ...
        try:
            ...
            result = (
                self.supabase.table("brain_memory")
                .insert(
                    {
                        "memory_type": "thought",
    ...
    async def recall_memory(
        ...
        try:
            ...
            # Search in content
            result = (
                query_filter.filter("content", "cs", json.dumps({"search": query}))
    ...
    async def make_decision(self, context: Dict, options: List[Dict]) -> Dict:
        ...
        try:
            ...
            relevant_memories = await self.recall_memory(
                json.dumps(context), category=context.get("category", "decision")
            )

            # Analyze options based on past decisions
            past_decisions = (
                self.supabase.table("decision_log")
                .select("*")
                .filter("decision_type", "eq", context.get("type", "general"))
            ...
            for option in options:
                score = await self._score_option(
                    option, relevant_memories, past_decisions.data
                )
    ...
    async def _reinforce_pattern(self, decision_id: str):
        ...
        if result.data:
            ...
            related_memories = await self.recall_memory(
                json.dumps(decision["input_context"]), limit=5
            )
    ...
    async def synthesize_knowledge(self, topic: str) -> Dict:
        ...
        try:
            ...
            patterns = (
                self.supabase.table("pattern_library")
                .select("*")
                .filter("pattern_data", "cs", json.dumps({"topic": topic}))
                .execute()
            )
            ...
            decisions = (
                self.supabase.table("decision_log")
                .select("*")
                .filter("input_context", "cs", json.dumps({"topic": topic}))
                .execute()
            )
            ...
            if memories:
                ...
                synthesis["key_insights"] = [
                    m["content"] for m in important_memories[:5]
                ]
                synthesis["confidence"] = np.mean([m["confidence"] for m in memories])

            # Store synthesis
            result = (
                self.supabase.table("knowledge_synthesis")
                .insert(
                    {
                        "topic": topic,
                ...
                if old_memories.data:
                    ...
                    for memory in old_memories.data:
                        ...
                        if new_importance < 0.1:
                            # Archive or delete
                            self.supabase.table("brain_memory").delete().eq(
                                "id", memory["id"]
                            ).execute()
                        else:
                            self.supabase.table("brain_memory").update(
                ...
                if recent_decisions.data:
                    ...
                    for dtype, decisions in decision_types.items():
                        if len(decisions) > 3:
                            ...
                            success_rate = sum(
                                1 for d in decisions if d.get("success")
                            ) / len(decisions)
    ...
    async def _knowledge_synthesis(self):
        ...
        while True:
            try:
                ...
                objectives = (
                    self.supabase.table("learning_objectives")
                    .select("*")
                    .filter("progress", "lt", 1.0)
                ...
                if objectives.data:
                    for objective in objectives.data:
                        ...
                        self.supabase.table("learning_objectives").update(
                            {"progress": progress, "current_knowledge": synthesis}
    ...
    async def _score_option(
        self, option: Dict, memories: List[Dict], past_decisions: List[Dict]
    ) -> float:
    ...
    async def _identify_thought_patterns(self) -> List[Dict]:
        ...
        for theme, count in themes.items():
            if count > 3:
                patterns.append(
                    {"pattern": "repeated_theme", "theme": theme, "frequency": count}
                )
    ...
    async def get_brain_status(self) -> Dict:
        ...
        # Get memory stats
        memory_count = (
            self.supabase.table("brain_memory").select("count", count="exact").execute()
        )
        thought_count = (
            self.supabase.table("thought_stream")
            .select("count", count="exact")
            .execute()
        )
        decision_count = (
            self.supabase.table("decision_log").select("count", count="exact").execute()
        )
        pattern_count = (
            self.supabase.table("pattern_library")
            .select("count", count="exact")
