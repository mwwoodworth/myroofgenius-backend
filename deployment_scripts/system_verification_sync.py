#!/usr/bin/env python3
"""
System Verification for Perplexity Audit - Synchronous Version
...
class SystemVerifier:
    ...
    def verify_all_systems(self):
        ...
        self.check_langgraph_workflows()
        
        # 3. Check AUREA status
        self.check_aurea_system()
        
        # 4. Check Claude Sub-Agent pipeline
        self.check_claude_subagents()
    ...
    def check_langgraph_workflows(self):
        """Check LangGraph workflows visibility"""
        print("\n🤖 Checking LangGraph Workflows...")
        
        try:
            response = requests.get(f"{self.base_url}/api/v1/langgraphos/status", timeout=10)
    ...
    def check_aurea_system(self):
        """Check AUREA system status"""
        print("\n🧠 Checking AUREA System...")
        
        try:
            # Check status
            status_response = requests.get(f"{self.base_url}/api/v1/aurea/status", timeout=10)
            
            # Check capabilities
            capabilities_response = requests.get(f"{self.base_url}/api/v1/aurea/capabilities", timeout=10)
            ...
            if status_response.status_code == 200:
                ...
                self.results["checks"]["aurea"] = {
                    ...
                    "roles": list(capabilities_data.get("roles", {}).keys()) if capabilities_data else [],
                    "success": True
                }
    ...
    def check_claude_subagents(self):
