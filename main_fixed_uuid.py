...
async def lifespan(app: FastAPI):
    logger.info("🚀 Starting BrainOps Backend v20.4.0 - UUID Fix Applied")
...
@app.get("/")
async def root():
    return {"message": "BrainOps API v20.4.0 - All Systems Operational"}
    ...
    try:
        ...
        try:
            ...
            if len(customer_id) == 36 and '-' in customer_id:
                ...
                result = db.execute(
                    text("SELECT * FROM customers WHERE id = :id::uuid"),
                    {"id": customer_id}
                ).fetchone()
            else:
                result = None
    ...
    try:
        ...
        db.execute(
            text("""
                ...
                VALUES (:id::uuid, :name, :email, :phone, :address, :city, :state, :zip, NOW(), NOW())
            """),
            {
                "id": customer_id,
                "name": customer.name,
...
async def delete_customer(customer_id: str, db: Session = Depends(get_db)):
    ...
    try:
        result = db.execute(
            text("DELETE FROM customers WHERE id = :id::uuid RETURNING id"),
            {"id": customer_id}
        )
        if result.rowcount == 0:
            raise HTTPException(status_code=404, detail=f"Customer {customer_id} not found")
    ...
    try:
        ...
        try:
            if len(job_id) == 36 and '-' in job_id:
                uuid.UUID(job_id)
                result = db.execute(
                    text("SELECT * FROM jobs WHERE id = :id::uuid"),
                    {"id": job_id}
                ).fetchone()
            else:
                result = None
    ...
    try:
        ...
        db.execute(
            text("""
                ...
                VALUES (:id::uuid, :customer_id::uuid, :title, :description, :status, :total_amount, NOW(), NOW())
            """),
            {
                "id": job_id,
...
async def redoc_docs():
    """ReDoc documentation"""
    return {"message": "ReDoc documentation available", "version": "20.4.0"}
