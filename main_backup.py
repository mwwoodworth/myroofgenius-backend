...
engine = create_engine(
    ...
    max_overflow=100,  # Increased from 50 to handle spikes
    pool_pre_ping=True,
...
async def lifespan(app: FastAPI):
    logger.info("🚀 Starting BrainOps Backend v65.0.0 - Recruitment Added")
    ...
    logger.info("🤖 AI systems fully activated and autonomous")
    yield
...
app = FastAPI(
    ...
    version="65.0.0",  # v65.0.0 with recruitment
    lifespan=lifespan
)
...
async def initialize_ai_brain():
    ...
    if AI_BRAIN_AVAILABLE:
        try:
            ...
            logger.info("✅ AI Brain Core initialized and operational")
        except Exception as e:
            logger.error(f"⚠️ AI Brain Core initialization failed: {e}")
            ai_brain = None
...
@app.get("/")
async def root():
    return {"message": "BrainOps API v30.3.0 - All Systems Operational"}

@app.get("/health")
async def render_health_check():
    """Simple health check for Render"""
    return {"status": "healthy", "version": "47.0.0"}
...
# Import and include Fixed Lead Capture endpoint
try:
    from fix_lead_capture import router as lead_capture_router
    app.include_router(lead_capture_router)
    print("✅ Fixed Lead Capture endpoints loaded - Async database handling")
except Exception as e:
    print(f"⚠️ Fixed Lead Capture not loaded: {e}")
    import traceback
    traceback.print_exc()
