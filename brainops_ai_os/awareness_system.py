...
class AwarenessSystem:
    ...
    def __init__(self, controller: "MetacognitiveController"):
        ...
        self.readings: Dict[SensorType, deque] = {
            sensor_type: deque(maxlen=1000) for sensor_type in SensorType
        }
    ...
    async def _start_sensors(self):
        ...
        self._tasks.append(
            self._create_safe_task(
                self._external_services_sensor(), name="external_services_sensor"
            )
        )
        ...
        while not self._shutdown.is_set():
            try:
                ...
                if (
                    ...
                    self.alerts[
                        db_error_key
                    ].resolution = f"Database recovered. Query time: {query_time:.0f}ms"
